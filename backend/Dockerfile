...
WORKDIR /src
COPY ["DigiMem.csproj", "."]
RUN dotnet restore "./DigiMem.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./DigiMem.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Bu aşama, son aşamaya kopyalanacak hizmet projesini yayımlamak için kullanılır
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./DigiMem.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
